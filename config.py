...
class Config:
	def __init__(self):
		self.model_filename = 'Transformer_adapter_inf_2layer_12head_16token.csv' # 'user_defined_example_small.csv','user_defined_example.csv', 'hdvit_changed.csv'
	...
	def load_model(self):
		try:
			with open(self.model_filename, mode='r', newline='') as file:
				...
				elif (first_row[1] == "Transformer_inf" or first_row[1] == "Transformer_adapter_inf"):
					for row in csv_reader:
						row = row[:-1]
